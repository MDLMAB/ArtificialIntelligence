...
class StackFrontier():
    ...
    def remove(self):   # remove the last node from the frontier
        if self.empty():
...
class QueueFrontier(StackFrontier): # QueueFrontier is a subclass of StackFrontier

    def remove(self):  # remove the first node from the frontier
        if self.empty():
            raise Exception("empty frontier")
        else:
...
class Maze(): # Maze class
    ...
    def __init__(self, filename):   # constructor
        """Read and store maze from a file."""
